<template>
    <div class="main-container">
        ...
        <form @submit.prevent="getSelectedData">
            ...
            <div>
                <button type="submit" class="submit-btn button-custom">submit</button>
                <button @click="reset" class="reset-btn button-custom">reset</button>
                <div class="dropdown d-inline">
                    <button
                        class="btn btn-secondary dropdown-toggle button-custom"
                        type="button"
                        id="dropdownMenu"
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenu">
                        <li>
                            <button class="dropdown-item" v-on:click="showType = 'TOTAL'">
                                Total
                            </button>
                        </li>
                        <li>
                            <button class="dropdown-item" v-on:click="showType = 'RECEIVE'">
                                Reveive
                            </button>
                        </li>
                        <li>
                            <button class="dropdown-item" v-on:click="showType = 'REDEEM'">
                                Redeem
                            </button>
                        </li>
                    </ul>
...
export default {
    ...
    methods: {
        async fetchAllUser() {
            await ShopStore.dispatch('fetchAllUserExceptAdmin');
            this.allUser = ShopStore.getters.getAllUserExceptAdmin;
        },
        print(something) {
                ...
                for (var i in this.allUser) {
                    ...
                    for (var j in this.allUser[i].points) {
                        ...
                        if (date >= start && date <= end) {
                            if (this.showType == 'TOTAL') {
                                type === 'RECEIVE'
                                    ? (data.point += amount)
                                    : (data.point -= amount);
                            } else if (this.showType == 'RECEIVE') {
                                if (type == 'RECEIVE') data.point += amount;
                            } else if (this.showType == 'REDEEM') {
                                if (type == 'REDEEM') data.point += amount;
                            }
                        }
